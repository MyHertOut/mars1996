    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="180px" label-position="left">
      ...
      <div class="unit-price-box">
        <el-form-item label="Trade Period:" prop="orderStartTime">
          <el-date-picker v-model="ruleForm.orderStartTime" type="datetime" :default-time="defaultTime" :default-value="new Date()" :picker-options="startDateOp"
            placeholder="Start date" style="width: 227px;" @focus="startTimeFocus" @change="startTimeFocus"></el-date-picker>
        </el-form-item>
        <el-form-item prop="orderEndTime" class="currency">
          <el-date-picker v-model="ruleForm.orderEndTime" type="datetime" :default-time="defaultTime" :picker-options="endDateOp"
...
export default {
  ...
  methods: {
    ...
    async getTokenCode () {
      this.loading = true;
      const data = await absPreOfferOrder({
        slient: true,
        notify: notify.error
      });
      this.loading = false;
      this.tokenCodeList = data.data.tokenList;
