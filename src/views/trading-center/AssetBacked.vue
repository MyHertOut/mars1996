<template>
  <div class="bond-trade" v-loading="loading">
    <sub-header title="Asset-Backed Trading" :menus="subMenus" @handleClick="handleClick" navTitle="Trade Centre">
      <div slot="btns">
        <el-button @click="$router.push({ name: 'AbsTradeHistory' })" class="history" v-permissions="'trade.centre.bond.abs.history'">History</el-button>
        <el-button type="primary" @click="$router.push({ name: 'AbsNewTrade' })" class="new-trade" v-permissions="'trade.centre.bond.new.abs.trade'">New Trade</el-button>
      </div>
    </sub-header>
    <div class="main-container">
      <div class="tabel-container">
        ...
        <custom-table :datas="tableDatas" :columns="tableColumns" rowKey="rowKey" headTitleProp="orderName" :pager="pager"
          ...
          <div slot="column-bar" class="operation-box" v-if="activesubMenu === 2 && ((scope.row.bizStatus === 3 && scope.row.paymentType === 1) || (scope.row.bizStatus === 8 && scope.row.paymentType === 2) || scope.row.bizStatus === 8 && scope.row.paymentType === 1)" slot-scope="scope">
            <el-button type="primary" class="btn" @click="receivedPayMent(scope.row)" v-permissions="'trade.centre.bond.abs.received.payment'"
            ...
            <el-button type="primary" class="btn" @click="receivedTransact(scope.row)" v-permissions="'trade.centre.bond.abs.received.transact'"
              v-if="scope.row.bizStatus === 8 && scope.row.paymentType === 1" :disabled="btnDis(scope.row.orderEndTime)">Transact</el-button>
          </div>
          <div slot="column-bar" class="operation-box" v-else-if="scope.row.bizStatus === 4 && activesubMenu === 1 || scope.row.bizStatus === 3 && activesubMenu === 1" slot-scope="scope" v-permissions="'trade.centre.bond.abs.created.confirm'">
            <el-button type="primary" class="btn" @click="createConfirm(scope.row)" v-if="scope.row.bizStatus === 4">Confirm</el-button>
          </div>
        </custom-table>
      </div>
    </div>
    ...
    <!-- transaction弹窗 -->
    <transaction-dialog :isShow="transactionDialog" :data="currentItem" :rejectLoading="rejectLoading" @closeDialog="transactionDialog = false" :abs="true"
...
import { getAbsTradeCreatedList, getAbsTradeReceivedList, absTradePayment, getBidBizStatus,
  absAcceptBidOrder, absRejectBidOrder, absQueryPaymentInfo, absTradeConfirm } from '@/api';
...
export default {
      ...
      subMenus: [
        {
          name: 'Created',
          key: 'created',
          functionCode: 'trade.centre.bond.abs.created'
        }, {
          name: 'Received',
          key: 'received',
          functionCode: 'trade.centre.bond.abs.received'
        }
      ],
      statusList: [],
      tableDatas: [],
  ...
  methods: {
    ...
    async getCreateList () {
      this.loading = true;
      this.tableDatas = [];
      const data = await getAbsTradeCreatedList({
        data: {
          ...this.searchParams,
          ...this.pageObj
        },
    ...
    async getReceivedList () {
      this.loading = true;
      this.tableDatas = [];
      const data = await getAbsTradeReceivedList({
        data: {
          ...this.searchParams,
          ...this.pageObj
        },
