  ...
  <div class="disboinds-parameters" v-loading="loading">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm"
      ...
      <el-form-item label="Company LEI：" prop="companyLei">
        <el-input v-model.trim="ruleForm.companyLei" autocomplete="off" maxlength="36" placeholder="Please enter the company name"></el-input>
      </el-form-item>
      <el-form-item label="Debt Seniority：" prop="debtSeniority">
        ...
        <!-- <el-input v-model="ruleForm.debtSeniority" autocomplete="off" maxlength="256"></el-input> -->
      </el-form-item>
    ...
    <el-form :model="ruleForm" :rules="listRules" ref="ratingForm"
      ...
      >
        <el-input v-model="ruleForm.issueRating" @blur="$refs.ratingForm.rules.issueRating[0].required = false;" autocomplete="off" maxlength="16" :disabled="ruleForm.ratingList.length >= 10"></el-input>
    ...
    <div class="form-btns">
      <el-button @click="$router.push({ path: '/issuance/bonds' })">Cancel</el-button>
      <el-button type="primary" @click="handleNext">Next</el-button>
    </div>
  </div>
</template>
<script>
import { newSecurity, getSecuritySMTInfo, getSecurityInfo } from '@/api/distribution-center';
...
export default {
  ...
  watch: {
    ...
    'ruleForm.companyLei': function (n, o) {
      if (/^[0-9a-zA-Z]+$/.test(this.ruleForm.companyLei) || n === '') {
        this.ruleForm.companyLei = n.toUpperCase();
      } else {
        this.ruleForm.companyLei = o;
      }
    },
  ...
  data () {
    return {
      ...
      debtSeniorityList: debtSeniorityList,
      rules: {
    ...
    async getBeforeInfo () {
      if (this.$route.query.serialNo) {
        ...
        if (data.code === '1000') {
          ...
          if (data.data.uniqueIdentifierCode) {
            this.ruleForm.identifierCodeList = data.data.uniqueIdentifierCode.split(',').map(v => { return { value: v }; });
          } else {
    ...
    handleNext () {
      this.$refs.ruleForm.validate(async valid => {
        ...
        if (valid && rating && identifier) {
          ...
          if (data.code === '1000') {
            this.$router.push({ name: 'DisIssueBondsInformation', query: { serialNo: data.data } });
          }
          this.loading = false;
        }
      });
