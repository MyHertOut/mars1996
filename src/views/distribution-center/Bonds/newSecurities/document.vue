<template>
  <div class="add-new-document" v-loading="loading">
    ...
    >
      ...
      >
        ...
        <div class="add-btn" v-if="ruleForm.privacyUploadList.length < 10" @click="privacyAddFile">
          <img src="../../../../assets/images/add.png" width="26" height="26" alt="png">
        </div>
      </el-form-item>
    ...
    <div class="form-btns">
      <el-button @click="$router.push({ name: 'DisIssueBondsPayment', query: { serialNo: $route.query.serialNo } })">Previous</el-button>
...
<script>
import { getSecurityInfo, newSecurity } from '@/api/distribution-center';
...
export default {
  ...
  methods: {
    async handleNext () {
      this.loading = true;
      const params = {
        privacyFile: this.ruleForm.privacyUploadList.filter(x => x.id).map(v => ({
          id: v.id,
          name: v.name,
          suffix: v.suffix
        })),
        publicFile: this.ruleForm.publicUploadList.filter(x => x.id).map(v => ({
          id: v.id,
          name: v.name,
          suffix: v.suffix
        })),
      ...
      const data = await newSecurity({ data: params, slient: true });
      if (data.code === '1000') {
        this.$router.push({ name: 'DisIssueBondsRestrictions', query: { serialNo: data.data } });
      }
      this.loading = false;
    },
    async getBeforeInfo () {
      if (this.$route.query.serialNo) {
        ...
        // this.loading = false;
        if (data.data.privacyFile) {
          const privacyFile = data.data.privacyFile; // JSON.parse(data.data.privacyFile) || [];
        ...
        if (data.data.publicFile) {
          const publicFile = data.data.publicFile; // JSON.parse(data.data.publicFile) || [];
    ...
    changeUpload (v, file) {
      ...
      reader.onload = async () => {
        ...
        const resData = await axios({
          ...
          v.loading = false;
          file.target.value = null;
          this.$message({ message: res.message || 'Network Error', type: 'error' });
        });
        if (!resData) return;
