<template>
  <div class="restrictions" v-loading="loading">
    ...
    <div class="country-box">
      <div class="country-content">
        ...
        <div class="country-item">
          ...
          <el-select v-model="instituCountry.country" filterable :disabled="!institutional" :loading="countryLoading">
            <el-option
        ...
        <div class="country-item">
          ...
          <el-select v-model="instituCountry.residency" filterable :disabled="!institutional" :loading="countryLoading">
            <el-option
    ...
    <div class="select-country" v-if="instituCountryData.filter(x => x.permitted == 2).length">
      <p class="sub-title">Prohibited</p>
    ...
    <div class="country-box">
      <div class="country-content">
        ...
        <div class="country-item">
          ...
          <el-select v-model="indiviCountry.country" :disabled="!individual" filterable :loading="countryLoading">
            <el-option
        ...
        <div class="country-item">
          ...
          <el-select v-model="indiviCountry.residency" :disabled="!individual" filterable :loading="countryLoading">
            <el-option
    ...
    <div class="form-btns">
      <el-button @click="$router.push({ name: 'DisIssueAbsDocument', query: { serialNo: $route.query.serialNo } })">Previous</el-button>
      <el-button type="primary" @click="handleNext">Next</el-button>
    </div>
  </div>
</template>
<script>
import { getAbsInfo, getMemberCountry, newAbsAdd } from '@/api/distribution-center';
...
export default {
  ...
  watch: {
    ...
    // countryOfIncorporation (n, o) {
    //   if (n === 1) { // Permitted
    //     this.incorporationCountryList.splice(0, 0, { enName: 'All', value: 'all' });
    //   } else {
    //     this.countryIncorporationItemList.forEach((val, key) => {
    //       if (val.value === 'All' && !val.isDefault) {
    //         this.countryIncorporationItemList.splice(key, 1);
    //         this.countryIncorporationItemList[0].value = ''; // 'China';
    //       }
    //     });
    //     this.incorporationCountryList.splice(0, 1);
    //   }
    // },
    // countryOfOperation (n, o) {
    //   if (n === 1) {
    //     this.operationCountryList.splice(0, 0, { enName: 'All', value: 'all' });
    //   } else {
    //     this.countryOperationItemList.forEach((val, key) => {
    //       if (val.value === 'All' && !val.isDefault) {
    //         this.countryOperationItemList.splice(key, 1);
    //         this.countryOperationItemList[0].value = ''; // 'China';
    //       }
    //     });
    //     this.operationCountryList.splice(0, 1);
    //   }
    // },
    nationality (n, o) {
      if (n === 1) {
  ...
  methods: {
    ...
    queryMemberCountry () {
      ...
      getMemberCountry().then(res => {
        ...
        if (code === '1000') {
          ...
          this.residencyCountryList.splice(0, 0, { enName: 'ALL', shortName: 'ALL', value: 'ALL' });
          this.incorporationCountryList.splice(0, 0, { enName: 'ALL', shortName: 'ALL', value: 'ALL' });
          this.operationCountryList.splice(0, 0, { enName: 'ALL', shortName: 'ALL', value: 'ALL' });
          this.nationalityCountryList.splice(0, 0, { enName: 'ALL', shortName: 'ALL', value: 'ALL' });
        }
      }).catch(() => {
        this.countryLoading = false;
      });
    ...
    handleNext () {
      ...
      this.loading = true;
      newAbsAdd({ data: { ...finData2, serialNo: this.$route.query.serialNo }, slient: true }).then(res => {
        this.loading = false;
        const { code, data } = res.data;
        if (code === '1000') {
          this.$router.push({
            name: 'DisIssueAbsViewdetail',
            query: { type: 'Step', serialNo: data }
          });
        }
      }).catch(() => {
        this.loading = false;
