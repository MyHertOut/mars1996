  ...
  <div class="payment" v-loading="loading">
    <el-form :model="ruleForm" ref="ruleForm" label-width="210px"
      ...
      <!-- <el-form-item label="Coupon Rate：" prop="couponRate" class="coupon-rate">
        <el-input v-model="ruleForm.couponRate" autocomplete="off">
          <span slot="suffix">%</span>
        </el-input>
      </el-form-item> -->
      <el-form-item label="Coupon Frequency：" prop="couponFrequency">
        <el-select v-model="ruleForm.couponFrequency" placeholder="">
      ...
      <!-- <el-form-item label="Coupon Periods：" prop="couponPeriods">
        <el-input v-model="ruleForm.couponPeriods" maxlength="4"></el-input>
      </el-form-item> -->
      ...
      <!-- <el-form-item label="Redemption Date：" prop="redemptionDate">
      ...
      <el-form-item label="Callable Bond：" prop="callable">
        ...
        <el-date-picker
          ...
          :disabled="ruleForm.callableDateList.length >= 10"
        ></el-date-picker>
        <div class="add-bond-btn" v-if="ruleForm.callableDateList.length < 10" @click="addCallableDate">
      ...
      <!-- <el-form :model="ruleForm" ref="callableDateForm" label-width="180px"
       ...
       <el-form-item label="Disbursement Token：" prop="disbursementMethod">
    ...
    <div class="form-btns">
      <el-button @click="$router.push({ name: 'DisIssueAbsInformation', query: { serialNo: $route.query.serialNo } })">Previous</el-button>
...
import { getDisbursementAddress, getAbsInfo, newAbsAdd } from '@/api/distribution-center';
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        disbursementCode: [{ required: true, trigger: 'blur', message: 'This field is required.' }],
  ...
  watch: {
    ...
    //   times = 31104000000;
    // }
    // const addTimes = times * parseInt(this.ruleForm.couponPeriods);
    // this.ruleForm.maturityDate = moment(this.ruleForm.firstSettlementDate.getTime() + addTimes).format('YYYY-MM-DD HH:mm:ss');
    // this.ruleForm.redemptionDate = this.ruleForm.maturityDate;
    // this.ruleForm.paymentDateTable = [];
    // for (let i = 1; i <= this.ruleForm.couponPeriods; i++) {
    //   const tempTime = times * parseInt(i);
    //   this.ruleForm.paymentDateTable.push({
    //     periods: i + '/' + this.ruleForm.couponPeriods,
    //     time: moment(this.ruleForm.firstSettlementDate.getTime() + tempTime).format('YYYY-MM-DD HH:mm:ss')
    //   });
    // }
    ...
    //   }
    //   if (this.ruleForm.firstSettlementDate && n !== '' && n !== '0') {
    //     this.ruleForm.couponPeriods = n.toString().replace(/[^\d]/g, '');
    //     let times;
    //     if (this.ruleForm.couponFrequency === '0') {
    //       times = 7776000000;
    //     } else if (this.ruleForm.couponFrequency === '1') {
    //       times = 15552000000;
    //     } else {
    //       times = 31104000000;
    //     }
    //     const addTimes = times * parseInt(n);
    //     this.ruleForm.maturityDate = moment(this.ruleForm.firstSettlementDate.getTime() + addTimes).format('YYYY-MM-DD HH:mm:ss');
    //     this.ruleForm.redemptionDate = this.ruleForm.maturityDate;
    //     this.ruleForm.paymentDateTable = [];
    //     for (let i = 1; i <= this.ruleForm.couponPeriods; i++) {
  ...
  methods: {
    ...
    async getBeforeInfo () {
      if (this.$route.query.serialNo) {
        ...
        if (code === '1000') {
          ...
          this.ruleForm.disbursementMethod = data.disbursementMethod ? '' + data.disbursementMethod : '0';
          // data.disbursementToken ? data.disbursementToken.split(',')[0] : '1';
          this.ruleForm.chooseCurrency = data.disbursementToken ? data.disbursementToken.split(',')[1] : ''; // 'USD';
          this.$nextTick(() => {
            // this.ruleForm.currencyToken = data.data.disbursementToken ? data.data.disbursementToken.split(',')[2] : '';
            this.ruleForm.disbursementCode = data.disbursementCode;
          });
          // this.ruleForm.disbursementCode = data.disbursementCode;
          this.ruleForm.callable = data.callable === true;
          this.ruleForm.callableDateList = data.callDateList ? data.callDateList.map(v => { return { value: moment(parseInt(v)).format('YYYY-MM-DD HH:mm:ss') }; }) : [];
          // this.ruleForm.callableDateList = data.callableDate ? data.callableDate.split(',').map(v => { return { value: moment(parseInt(v)).format('YYYY-MM-DD HH:mm:ss') }; }) : [];
          this.ruleForm.firstSettlementDate = data.firstSettlementDate ? new Date(data.firstSettlementDate) : '';
          this.ruleForm.couponFrequency = data.couponFrequency;
          if (!data.disbursementTokenAddress) {
    ...
    changeFirstDate (data) {
      ...
      //   const addTimes = times * parseInt(this.ruleForm.couponPeriods);
      //   this.ruleForm.maturityDate = moment(data.getTime() + addTimes).format('YYYY-MM-DD HH:mm:ss');
      //   this.ruleForm.redemptionDate = this.ruleForm.maturityDate;
      //   this.ruleForm.paymentDateTable = [];
      //   for (let i = 1; i <= this.ruleForm.couponPeriods; i++) {
      //     const tempTime = times * parseInt(i);
      //     this.ruleForm.paymentDateTable.push({
      //       periods: i + '/' + this.ruleForm.couponPeriods,
      //       time: moment(data.getTime() + tempTime).format('YYYY-MM-DD HH:mm:ss')
      //     });
      //   }
      //   const lastPeriods = this.ruleForm.paymentDateTable[this.ruleForm.paymentDateTable.length - 1].periods;
      //   this.ruleForm.paymentDateTable[this.ruleForm.paymentDateTable.length - 1].periods = lastPeriods + ' (Redemption)';
    ...
    addCallableDate () {
      this.$refs.ruleForm.validateField('callableDate');
      if (!this.ruleForm.callableDate) {
        return;
      }
      ...
      if (filterArr.length <= 0) {
        ...
        const arr = this.callableDateList.map(v => v.value);
        arr.sort((a, b) => {
          return a - b;
        });
    ...
    next () {
      this.$refs.ruleForm.validate(async valid => {
        ...
        if (valid && totalValid) {
          ...
          params.callDateList = this.ruleForm.callableDateList.map(v => new Date(v.value).getTime());
          ...
          this.loading = true;
          const resData = await newAbsAdd({ data: params, slient: true, notify: notify.error });
          if (!resData) return;

          const { code } = resData.data;
          if (code === '1000') {
            this.$router.push({ name: 'DisIssueAbsDocument', query: { serialNo: params.serialNo } });
          }
          this.loading = false;
        }
      });
