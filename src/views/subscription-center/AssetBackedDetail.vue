<template>
  <div class="subscription-detail" v-loading="loading">
      ...
      <div class="info-top">
          ...
          <div class="specific-information">
            ...
            <el-row>
              ...
              <el-col :span="12">
                ...
                <span class="value" v-if="detailObj.paymentType === 2">{{formatNumber(detailObj.price)}} {{isEmpty(detailObj.currency)}}</span>
        ...
        <div class="detail-right">
          ...
          <!-- <div class="parameter-box">
            ...
            <span class="value" v-else>{{detailObj.annualEarnings}} {{detailObj.disbursementToken}}</span>
          </div> -->
          <div class="parameter-box">
    ...
    <el-dialog title="Payment" :visible.sync="offSubmitDialog" width="744px" :close-on-click-modal="false"
...
import subscriptionInfo from './absDetail/subscriptionInfo';
import basicInformation from './absDetail/basicInformation';
import principals from './absDetail/principals';
import tradingRestriction from './absDetail/tradingRestriction';
import documents from './absDetail/documents';
import { getAbsDetailInfo, multiAddresses, createAbsBidOrder, getTermsAndConditions, getBalance } from '@/api';
...
export default {
      ...
      breadData: [
        {
          ...
          name: 'Asset-Backed Subscription',
          to: '/asset-backed-subscription'
        }, {
          name: ''
        }
      ],
  ...
  watch: {
    ...
    'quantityForm.quantityNum': function (n, o) {
      if (n !== '') {
        ...
        this.detailObj.annualEarnings = operationNumber(parseFloat(n), parseFloat(this.detailObj.couponRate) / 100, 'multiply');
        if (n.split('.').length < 1) {
  ...
  methods: {
    offSubmitOrder () {
      this.$refs.offPayForm.validate(async valid => {
        if (valid) {
          ...
          const data = await createAbsBidOrder({
            data: {
    ...
    async getDetailInfo () {
      this.loading = true;
      const data = await getAbsDetailInfo({
        data: {
