<template>
  <div class="bond-subscription" v-loading="loading">
    <sub-header title="Asset-Backed Subscription" :menus="subMenus" @handleClick="handleClick" navTitle="SubScription Centre" />
    <div class="main-container">
      ...
      <div class="tabel-container">
        ...
        <custom-table :datas="tableDatas" :columns="tableColumns" rowKey="rowKey" headTitleProp="orderName" @handleRowClick="handleRowClick"
          ...
          <div slot="firstSettlementDate" slot-scope="scope">{{formatTime(scope.row.firstSettlementDate)}}</div>
...
import { getAbsPublicList, getAbsPrivateList } from '@/api';
...
export default {
  ...
  methods: {
    ...
    async privateList () {
      ...
      const data = await getAbsPrivateList({
        data: {
          ...this.searchParams,
          ...this.pageObj,
          ...this.sortObj
    ...
    async publicList () {
      this.loading = true;
      this.searchParams.bizStatus = this.tabsActive;
      const data = await getAbsPublicList({
        data: {
          ...this.searchParams,
          ...this.pageObj,
          ...this.sortObj
